...
* MJ: Alain created [the solid-test-suite organization](https://github.com/solid-test-suite). Should we move our repositories there?
...
* SC: Here are some proposed actions. In these proposals, I assume that:
  ...
  * Tests and reports that are not approved for publication at locations acknowledged by the Editors Team is out of scope for the Charter.
    ...
    * SC: Number of issues on the reports.
...
ACTION SC: Make issues for the charter proposals. Needs to leave room for unapproved reports / tests.

  * Software maintainers give consent for reports to be published.
  * Charter may be updated based on the needs of the CG.

ACTION: SC to create issues for PROPOSALs 1-3 so they can be discussed in a broader audience.
